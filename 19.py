...
def get_input_data():
    ...
    for i, dir_name in enumerate(dir_list):
        print
        "\t%d)%s" % ((i + 1), dir_name.split("scp_backup_from-")[1])
    print("*" * 50) + "\n"
    ...
    print
    ">>> %s가 선택되었습니다" % dir_path.split("scp_backup_from-")[1]
...
if __name__ == "__main__":
    ...
    if input_data != None:
        ...
        if len(scp_file_list) == 0:
            print
            "선택한 기간 동안 백업된 파일이 없습니다."
        elif len(scp_file_list) == 1:
            print
            "선택한 기간 동안 백업된 파일이 1건이어서 변경 사항을 비교할 수 없습니다."
        else:
            i = 0
